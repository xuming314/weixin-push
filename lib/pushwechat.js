 ...
 * ]
 */
Pusher.prototype.singleSendPicMsg = function (username, appMsgId, picMsgs, callback) {
    var self = this;
    if (self.cookie) {
        return proxy.singleSendPicMsg(username, appMsgId, picMsgs, self.cookie, self.token, function (err, data) {
            if (err) {
                self.cookie = null;
            }
            callback(err, data);
